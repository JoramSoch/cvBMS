function MS_SelMod_BMS(BMS, mask, extent) % Release cvBMS
% _
% Determine Selected Model after Bayesian Model Selection
% FORMAT MS_SelMod_BMS(BMS, mask, extent)
...
% 
% FORMAT MS_SelMod_BMS(BMS, mask, extent) creates voxel-wise selected-
...
%  Last edit: 15/09/2016, 08:45 (V0.9a/V13a)
...
% Init progress bar
%-------------------------------------------------------------------------%
Finter = spm('FigName','MS_SelMod_BMS: load');
...
% Init progress bar
%-------------------------------------------------------------------------%
Finter = spm('FigName','MS_SelMod_BMS: estimate');
...
% Init progress bar
%-------------------------------------------------------------------------%
Finter = spm('FigName','MS_SelMod_BMS: save');
...
%-------------------------------------------------------------------------%
folder = strcat(BMS_dir,'/','MS_SelMod_LF_',num2str(extent),'/');
mkdir(folder); cd(folder);

% Write images to disk
%-------------------------------------------------------------------------%
H.fname   = strcat('MS_SelMod_mod.nii');    % selected model
H.descrip = 'MS_SelMod_BMS: index of the selected model';
spm_write_vol(H,reshape(M_sel,H.dim));
H.fname   = strcat('MS_SelMod_ord.nii');    % order position
H.descrip = 'MS_SelMod_BMS: position of the selected model';
spm_write_vol(H,reshape(M_ord,H.dim));
for i = 1:sel_mods                          % selected models
    H.fname   = strcat('MS_SelMod_map','_pos_',MF_int2str0(i,ceil(log10(M+1))),'_mod_',MF_int2str0(mod_list(i,1),ceil(log10(M+1))),'_',mod_names{mod_list(i,1)},'.nii');
    H.descrip = sprintf('MS_SelMod_BMS: LF in voxels where %s has been selected', mod_names{i});
